...
function ShowFilms({ searchValue, setIdOfFilm }) {
        ...
        if (fetching) {
            ...
            fetch(`https://www.omdbapi.com/?apikey=edd125b0&s=${searchValue[0]}&type=${searchValue[1]}&page=${currentPage}`)
                ...
                .then(result => {
                    ...
                    if (!result.Search) throw new Error(result.Error);
                ...
                .catch(err => {
                    ...
                    console.log('ShowFilms - error', result);
                    setError(err + '')
    ...
    //     console.log('scrollHadler: films.length and totalCount', films.length, totalCount);
    //     console.log('scrollHadler: films.length < totalCount', films.length < totalCount);
    //     // console.log('Все условие: ', (evt.target.documentElement.scrollHeight - (evt.target.documentElement.scrollTop + window.innerHeight) < 100
    //     //     && (films.length < totalCount)) === true);
